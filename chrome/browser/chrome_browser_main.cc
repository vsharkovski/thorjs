...
void ChromeBrowserMainParts::PostProfileInit(Profile* profile,
  ...
  if (is_initial_profile) {
    ...
    // Initialize RangeFileService and browser process RangeStore.
    auto shared_url_loader_factory = profile->GetURLLoaderFactory();
    thorzeel::RangeFileService::Create(std::move(shared_url_loader_factory));

    auto* range_file_service = thorzeel::RangeFileService::Get();
    mojo::Remote<thorzeel::mojom::RangeFileService> remote;
    mojo::PendingReceiver<thorzeel::mojom::RangeFileService> receiver =
        remote.BindNewPipeAndPassReceiver();
    LOG(INFO) << "PostProfileInit: Adding browser-side receiver to "
                 "RangeFileService, creating RangeStore";
    range_file_service->AddReceiver(std::move(receiver));
    thorzeel::RangeStore::Create(std::move(remote));
  }
