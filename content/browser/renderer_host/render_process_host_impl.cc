...
void RenderProcessHostImpl::RegisterMojoInterfaces() {
  ...
  AddUIThreadInterface(
      registry.get(),
      base::BindRepeating(
          [](mojo::PendingReceiver<thorzeel::mojom::RangeFileService>
                 receiver) {
            auto* range_file_service = thorzeel::RangeFileService::Get();
            if (!range_file_service) {
              LOG(ERROR) << "RegisterMojoInterfaces: No range file service";
              return;
            }
            LOG(INFO) << "RegisterMojoInterfaces: Adding renderer receiver to "
                         "RangeFileService";
